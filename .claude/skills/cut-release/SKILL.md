...
name: cut-release
description: >-
  Cut (publish) a release of this academy by publishing the current Release
  Drafter draft with the gh CLI. Use this whenever the user asks to "cut a
  release", "publish the release", "ship a release", "release the academy", "do
  a release", "release the latest merge", or wants the recently merged PR to go
  ...
  build-and-publish workflow rebuilds the site and pushes it to the Layer5
  Academy platform. The version tag is already set by Release Drafter and
  auto-increments after each release, so this skill never creates or bumps a tag.
---

# Cut an academy release
...
by GitHub Actions - for a content repo like this, publishing the draft *is* the whole
release.
...
- Publishing that draft (the release `published` event) triggers
  `.github/workflows/build-and-release.yml`, which runs the production Hugo build and
  publishes the rendered academy content to the Layer5 Academy platform (via the
  `layer5io/academy-build` action). The published release tag is the version passed to
  that publish step.
...
Flipping `--draft=false` publishes it and fires the `published` event that drives the academy
build-and-publish. Mark it `--latest` so it carries the "Latest" badge.
...
version to the user and note that the build-and-publish workflow has been triggered by the
release event and will rebuild the site and push it to the Layer5 Academy platform.
